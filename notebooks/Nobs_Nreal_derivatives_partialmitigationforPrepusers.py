...
from tapm import model_STI_partialmitigationforP as model
import icomo
...
xi = 0.5  # partial mitigation for people on PrEP
...
def calc_equilibrium(H, P, lambdaP, betaSTI):
    ...
    args_mod["xi"] = xi  # partial mitigtaion for people on PrEP

    # run the model fro 80 years (long time to get to steady state)
    output = icomo.diffeqsolve(args = args_mod, ODE = model.main_model, y0 = y0, ts_out = np.linspace(0, 365*80, 365*80+1), max_steps=365*80+1)
    ...
    return y1["Ia_STI"], y1["Is_STI"], y1["S_STI"], lambdas(P,H), lambdaa(P,H), np.sum(lambdas(P,H) * y1["Is_STI"] + lambdaa(P,H) * y1["Ia_STI"])
...
if not onlyplots:
    for lambdaP in lambdaPs:
        for betaSTI in betaSTIs:
            ...
            S_vals = np.zeros((len(Ps), len(Hs)))
            Ia_vals = np.zeros((len(Ps), len(Hs)))
            Is_vals = np.zeros((len(Ps), len(Hs)))
            lambdaa_vals = np.zeros((len(Ps), len(Hs)))
            lambdas_vals = np.zeros((len(Ps), len(Hs)))

            # calculate Nreal and Nobs
            for i,H in enumerate(Hs):
                for j,P in enumerate(Ps):
                    Ia_vals[i,j], Is_vals[i,j], S_vals[i,j], lambdas_vals[i,j], lambdaa_vals[i,j], Nobs_res[i,j] = calc_equilibrium(H, P, lambdaP, betaSTI)
                    Nreal_res[i,j] = Ia_vals[i,j] + Is_vals[i,j]
            ...
            with open(
                "../results/Nreal_Nobs_partialmitigationforP_xi%g_lambdap%g_betaSTI%g_dN%s.npy" %(xi, lambdaP * 360, betaSTI, derivative),"wb",) as f:
                np.save(f, Ps)
                np.save(f, Hs)
                np.save(f, dNrealdP)
                np.save(f, dNobsdP)
                ...
                np.save(f, Ia_vals)
                np.save(f, Is_vals)
                np.save(f, lambdaa_vals)
                np.save(f, lambdas_vals)
...
for i, betaSTI in enumerate(beta_STI_values):
    ...
    for lambdaP in lambda_P_values:
        # load data
        with open("../results/Nreal_Nobs_partialmitigationforP_xi%g_lambdap%g_betaSTI%g_dN%s.npy" %(xi, lambdaP * 360, betaSTI, derivative),"rb") as f:
            Ps_plot = np.load(f)
            Hs_plot = np.load(f)
...
for row_idx, beta_STI in enumerate(beta_STI_values):
    for col_idx, (lambda_P, label) in enumerate(zip(lambda_P_values, lambda_P_labels)):
        ...
        ax.set_xticklabels([int(Ps[0] * 100), int(Ps[50] * 100), int(Ps[100] * 100)], fontsize=8)
...
fig.savefig("../figures/final_figure_withinflux_partialmitigationforP_xi%g_DERIVATIVES_dN%s.pdf" %(xi, derivative), format="pdf", bbox_inches="tight")